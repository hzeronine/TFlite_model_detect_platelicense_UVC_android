    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        listAdapter = new ArrayAdapter<ListItem>(getActivity(), 0, listItems) {
            ...
            public View getView(int position, View view, @NonNull ViewGroup parent) {
                ...
                String deviceName = item.device.getProductName();
                if (view == null)
                    view = getActivity().getLayoutInflater().inflate(R.layout.device_list_item, parent, false);
                TextView text1 = view.findViewById(R.id.text1);
                TextView text2 = view.findViewById(R.id.text2);
                if(item.driver == null)
                    text1.setText(deviceName == null? "Unknown Device" : deviceName + ", <no driver>");
                else if(item.driver.getPorts().size() == 1)
                    text1.setText( item.device.getProductName() + ", driver: " + item.driver.getClass().getSimpleName().replace("SerialDriver",""));
                else
                    text1.setText(item.device.getProductName() + ", driver: " + item.driver.getClass().getSimpleName().replace("SerialDriver","")+", Port "+item.port);
